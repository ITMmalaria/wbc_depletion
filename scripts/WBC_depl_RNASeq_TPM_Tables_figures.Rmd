...
```{r}
info<-read.csv(here("./data/intermediate/WBCfiltration_RNAseq_samplemap_stats.csv"), header = TRUE)
```
...
ff <- list.files( path = here("./data/intermediate/TPM_rawoutput"), pattern = "*_TPMCalculator_genes.out", full.names = TRUE )
```

extract the data and collect as separate dataframes as lists in the object counts.files 
...
```{r}
gsub(".*-\\d*-\\d*_","",basename(ff))


ff1 <- gsub( "_TPMCalculator_genes[.]out", "", ff )
ff1 <- gsub( "C[:]/Users/ekattenberg/Documents/Werk_LOCAL/WBC filtration/TPM_coordsorted/TPM_rawoutput/104974-001-", "", ff1 )
```
