...
#         # extract properly paired count from samtools flagstat
#         proper_mapped_all=$(sed '12q;d' "${PROJECT_ROOT}/results/star_salmon/samtools_stats/${sample}.markdup.sorted.bam.flagstat" | sed 's/ \+.*//g')
#         proper_mapped_pk=$(sed '12q;d' "${PROJECT_ROOT}/results/star_salmon/samtools_stats_pk/${sample}.markdup.sorted.pk.bam.flagstat" | sed 's/ \+.*//g')
