...
output="${PROJECT_ROOT}/results-refseq/mapping_stats_star.csv"
...
tail -n +2 "${samplesheet}" | \
    while IFS=, read -r sample fastq_1 fastq_2 strandedness condition RNA_type library_kit removal WBC_depletion kit; do
        ...
        trim_log="${PROJECT_ROOT}/results-refseq/trimgalore/${sample}_1.fastq.gz_trimming_report.txt"
        ...
        trim_log="${PROJECT_ROOT}/results-refseq/trimgalore/${sample}_2.fastq.gz_trimming_report.txt"
        ...
        log="${PROJECT_ROOT}/results-refseq/star_salmon/log/${sample}.Log.final.out"
        ...
        unique_mapped_pk=$(samtools view "${PROJECT_ROOT}/results-refseq/star_salmon/${sample}.markdup.sorted.pk.bam" | awk '/NH:i:1\t/ {print $1} ' | sort | uniq | wc -l)

        # extract properly paired count from samtools flagstat
        flagstat_total_all=$(sed '1q;d' "${PROJECT_ROOT}/results-refseq/star_salmon/samtools_stats/${sample}.markdup.sorted.bam.flagstat" | sed 's/ \+.*//g')
        flagstat_total_pk=$(sed '1q;d' "${PROJECT_ROOT}/results-refseq/star_salmon/samtools_stats_pk/${sample}.markdup.sorted.pk.bam.flagstat" | sed 's/ \+.*//g')

        flagstat_primary_all=$(sed '2q;d' "${PROJECT_ROOT}/results-refseq/star_salmon/samtools_stats/${sample}.markdup.sorted.bam.flagstat" | sed 's/ \+.*//g')
        flagstat_primary_pk=$(sed '2q;d' "${PROJECT_ROOT}/results-refseq/star_salmon/samtools_stats_pk/${sample}.markdup.sorted.pk.bam.flagstat" | sed 's/ \+.*//g')

        flagstat_secondary_all=$(sed '3q;d' "${PROJECT_ROOT}/results-refseq/star_salmon/samtools_stats/${sample}.markdup.sorted.bam.flagstat" | sed 's/ \+.*//g')
        flagstat_secondary_pk=$(sed '3q;d' "${PROJECT_ROOT}/results-refseq/star_salmon/samtools_stats_pk/${sample}.markdup.sorted.pk.bam.flagstat" | sed 's/ \+.*//g')

        flagstat_primary_mapped_all=$(sed '8q;d' "${PROJECT_ROOT}/results-refseq/star_salmon/samtools_stats/${sample}.markdup.sorted.bam.flagstat" | sed 's/ \+.*//g')
        flagstat_primary_mapped_pk=$(sed '8q;d' "${PROJECT_ROOT}/results-refseq/star_salmon/samtools_stats_pk/${sample}.markdup.sorted.pk.bam.flagstat" | sed 's/ \+.*//g')

        flagstat_proper_pair=$(sed '12q;d' "${PROJECT_ROOT}/results-refseq/star_salmon/samtools_stats/${sample}.markdup.sorted.bam.flagstat" | sed 's/ \+.*//g')
        flagstat_proper_pair_pk=$(sed '12q;d' "${PROJECT_ROOT}/results-refseq/star_salmon/samtools_stats_pk/${sample}.markdup.sorted.pk.bam.flagstat" | sed 's/ \+.*//g')
