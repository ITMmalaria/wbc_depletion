...
star_stats <- read_delim(here("results-refseq/multiqc/star_salmon/multiqc_report_data/mqc_star_alignment_plot_1.txt"), delim = "\t")
```
...
samples <- read_csv(here("./data/samplesheet.csv"), col_select = c("sample", "condition", "RNA_type", "library_kit", "removal", "WBC_depletion", "kit"), col_types = "c") %>% 
  mutate(kit = str_replace_all(kit, "_", " ")) %>% 
  mutate(sample_name = str_replace(str_replace(sample, "104974-001-0", "Sample "), "_S1_L001", paste(" -", kit, "-", WBC_depletion))) %>% 
  mutate(sample_short = str_replace(str_replace(sample, "104974-001-0", "Sample "), "_S1_L001", ""))
...
gene_counts_txi %>% summarise(across(starts_with("X"), sum)) %>% to_long() %>% left_join(samples, by = "sample") %>% 
  ggplot(aes(x = sample_short, y = counts, fill = kit)) + 
  geom_bar(stat = "identity") + 
  scale_fill_viridis(option = "viridis", discrete = T, labels = c("mRNA Illumina", "mRNA Qiagen globin depletion", "mRNA Qiagen globin/rRNA depletion", "tRNA Qiagen globin/rRNA depletion")) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
...
pcr_dups %>% 
  ...
  ggplot(aes(x = sample_name, y = counts, fill = type)) +
      ...
      scale_x_discrete(limits = rev) +
      scale_y_continuous(labels = scales::label_number(scale_cut = cut_short_scale())) +
      labs(x = "Sample", y = "Number of reads", fill = "Deduplication stats")
```
