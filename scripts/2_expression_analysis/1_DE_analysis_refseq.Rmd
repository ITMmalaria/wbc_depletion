...
- Salmon gene-level quant files. TODO: remove these as they are redundant
...
- Gene-level counts inside DESeq/SummarizedExperiment object (created by manually importing Salmon `quant.sf` files via `tximport`. Values should correspond exactly to the other gene-level counts).
...
> For `tximport`, there are still several different ways of creating count matrices (i.e., using a length offset or by scaling the TPM abundances, but this should only be relevant for downstream differential expression testing TODO) See the description of different options provided by the nf-core/rnaseq pipeline: https://nf-co.re/rnaseq/3.14.0/docs/output/#pseudoalignment and the `tximport` docs here: https://bioconductor.org/packages/release/bioc/vignettes/tximport/inst/doc/tximport.html.

> The Salmon transcript-level quant files can also be imported using tximport and then converted into a DESeq object (~SummarizedExperiment), as done below. This approach provides the same count matrix as the tximport gene counts csv file, but it enables the DESeq2 approach for visualisation, namely transforming the counts using `vst` (variance stabilizing transformation) or `rlog` in order to remove the dependence of the variance on the mean. See [DESeq2 vignette](https://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#data-transformations-and-visualization).

TODO

The Salmon transcript-level counts can also be imported using `tximport` and then converted into a DESeq/SummarizedExperiment object, as done below, and then transformed using `vst` or `rlog` for visualization purposes. 

However, since we have many zero counts (e.g., rRNA being depleted in some samples), the transformation might not work as well as intended.

TODO: for low number of samples, and large differences between them (in our case, we expect many zero counts for e.g. depleted rRNA genes in some samples).
...
gene_counts_salmon <- annotate_gene_type(gene_counts_salmon, gene_annotation)
