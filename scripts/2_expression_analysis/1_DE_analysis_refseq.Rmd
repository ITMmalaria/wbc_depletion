...
smallestGroupSize <- 1
keep <- rowSums(counts(gse) >= 1) >= smallestGroupSize
...
keep <- rowSums(counts(gse_pk) >= 1) >= smallestGroupSize
...
**TODO: optionally repeat using low-count filtered gene lists.**
...
v <- draw_venn(gene_tpm_binary[comparison_centpip$sample],
               c("mRNA Qiagen - FSglob", "tRNA Qiagen - FSglobH", "mRNA Illumina"),
               "Centpip",
               "venn-centpip.png")
v

v <- draw_venn(gene_tpm_binary[comparison_plasmo$sample],
               c("mRNA Qiagen - Fsglob", "tRNA Qiagen - FSglobH", "mRNA Illumina"),
               "Plasmo",
               "venn-plasmo.png")
v

v <- draw_venn(gene_tpm_binary[comparison_pmacs$sample],
               c("mRNA Qiagen - FSglob", "tRNA Qiagen - FSglobH", "mRNA Illumina"),
               "Pmacs",
               "venn-pmacs.png")
v

v <- draw_venn(gene_tpm_binary[comparison_cellulose$sample],
               c("mRNA Qiagen - FSglob", "tRNA Qiagen - FSglobH", "mRNA Illumina"),
               "Cellulose",
               "venn-cellulose.png")
v
...
#   x = gene_tpm_pk_binary_list[comparison_centpip$sample],
#   category.names = c("mRNA Qiagen - FSglob", "tRNA Qiagen - FSglobH", "mRNA Illumina"),
#   # Circles
#   lwd = 2,
#   # lty = "blank",
#   # col = palette.colors(3),
...
```{r}
v <- draw_venn(gene_tpm_pk_binary_list[comparison_centpip$sample],
               c("mRNA Qiagen - FSglob", "tRNA Qiagen - FSglobH", "mRNA Illumina"),
               "Centpip",
               "venn-centpip_pf.png")
v

v <- draw_venn(gene_tpm_pk_binary_list[comparison_plasmo$sample],
               c("mRNA Qiagen - Fsglob", "tRNA Qiagen - FSglobH", "mRNA Illumina"),
               "Plasmo",
               "venn-plasmo_pf.png")
v

v <- draw_venn(gene_tpm_pk_binary_list[comparison_pmacs$sample],
               c("mRNA Qiagen - FSglob", "tRNA Qiagen - FSglobH", "mRNA Illumina"),
               "Pmacs",
               "venn-pmacs_pf.png")
v

v <- draw_venn(gene_tpm_pk_binary_list[comparison_cellulose$sample],
               c("mRNA Qiagen - FSglob", "tRNA Qiagen - FSglobH", "mRNA Illumina"),
               "Cellulose",
               "venn-cellulose_pf.png")
v
```
...
```{r}
txdb <- makeTxDbFromGFF(here("./data/ref-refseq-refseq/GCF_000001405.40_GRCh38.p14_genomic.gff.gz"), format = "gff3", organism = "Homo sapiens")
k <- keys(txdb, keytype = "TXNAME")
# if this step fails, make sure dplyr is not overriding the select() function,
# see https://github.com/lzcyzm/exomePeak/issues/1
tx2gene <- AnnotationDbi::select(txdb, k, "GENEID", "TXNAME")
...
```{r}
txdb <- makeTxDbFromGFF("./data/ref-refseq-refseq/GCF_000006355.2_GCA_000006355.2_genomic.gff.gz", format = "gff3", organism = "Plasmodium knowlesi")
k <- keys(txdb, keytype = "TXNAME")
# if this step fails, make sure dplyr is not overriding the select() function,
# see https://github.com/lzcyzm/exomePeak/issues/1
tx2gene <- AnnotationDbi::select(txdb, k, "GENEID", "TXNAME")
