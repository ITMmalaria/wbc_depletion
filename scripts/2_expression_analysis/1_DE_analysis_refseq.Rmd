...
```{r}
gene_counts_txi %>% summarise(across(starts_with("X"), sum)) %>% to_long() %>% left_join(samples, by = "sample") %>% 
  ggplot(aes(x = sample, y = counts, fill = kit)) + 
...
```{r}
pcr_dups <- read_delim(here("results-refseq/multiqc/star_salmon/multiqc_report_data/multiqc_picard_dups.txt"), delim = "\t")
...
pcr_dups %>% 
  ...
  select(c(Sample, READS_IN_UNIQUE_PAIRS, READS_IN_UNIQUE_UNPAIRED, READS_IN_DUPLICATE_PAIRS_OPTICAL, READS_IN_DUPLICATE_PAIRS_NONOPTICAL, READS_IN_DUPLICATE_UNPAIRED)) %>% 
