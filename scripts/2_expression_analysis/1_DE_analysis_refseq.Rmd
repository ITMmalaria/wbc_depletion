...
```{r}
gene_counts_txi %>% summarise(across(`104974-001-001_S1_L001`:`104974-001-016_S1_L001`, sum)) %>%
  to_long() %>% 
  left_join(samples, by = "sample") %>% 
...
gene_annotation %>%
  ...
  ggplot(aes(x = gene_name, y = counts, fill = sample_short)) +
    ...
    scale_x_discrete(limits = rev) +
    # labs(colour = "Sample") +
    ylab(label = "log(TPM+1)") + xlab(label = "Gene name") +
    facet_wrap(~sample) +
...
gene_tpm_binary_list <- purrr::set_names(colnames(gene_tpm_binary %>% select(`104974-001-001_S1_L001`:`104974-001-016_S1_L001`))) %>%
  purrr::imap(~ gene_tpm_binary$gene_id[gene_tpm_binary[, .x, drop = T]])

gene_tpm_pk_binary_list <- purrr::set_names(colnames(gene_tpm_pk_binary %>% select(`104974-001-001_S1_L001`:`104974-001-016_S1_L001`))) %>%
