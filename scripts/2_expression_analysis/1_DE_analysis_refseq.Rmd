...
# create filtered versions of deseq2 object
...
rld_pk_filtered <- rlog(gse_pk, blind = TRUE)
...
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- vsd_filtered$sample_name
colnames(sampleDistMatrix) <- NULL
pheatmap(sampleDistMatrix,
  clustering_distance_rows = sampleDists,
  clustering_distance_cols = sampleDists,
...
```{r, warning=FALSE}
sampleDists <- dist(t(assay(rld_filtered[rowData(rld_filtered)$species == "pk", ])))
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- rld_filtered$sample_name
colnames(sampleDistMatrix) <- NULL
pheatmap(sampleDistMatrix,
  ...
  main = expression(paste("Sample-to-sample Euclidian distance based on", italic("P. knowlesi"), "genes"))
)
```
...
```{r, warning=FALSE}
sampleDists <- dist(t(assay(vsd_filtered[rowData(vsd_filtered)$species == "pk", ])))
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- vsd_filtered$sample_name
colnames(sampleDistMatrix) <- NULL
pheatmap(sampleDistMatrix,
...
```{r, warning=FALSE}
sampleDists <- dist(t(assay(rld_pk_filtered)))
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- rld_pk_filtered$sample_name
colnames(sampleDistMatrix) <- NULL
pheatmap(sampleDistMatrix,
  clustering_distance_rows = sampleDists,
  clustering_distance_cols = sampleDists,
...
```{r, warning=FALSE}
sampleDists <- dist(t(assay(vsd_pk_filtered)))
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- vsd_pk_filtered$sample_name
colnames(sampleDistMatrix) <- NULL
pheatmap(sampleDistMatrix,
  clustering_distance_rows = sampleDists,
  clustering_distance_cols = sampleDists,
...
This plot clusters the top n=20 most variable genes centred on the gene's average expression level across all samples using the VST data.

```{r}
# select 20 most variable genes
topVarGenes <- head(order(rowVars(assay(vsd_filtered)), decreasing = TRUE), 20)
# select vst transformed data and center
mat <- assay(vsd_filtered)[topVarGenes, ]
mat <- mat - rowMeans(mat)
# add grouping columns
anno <- as.data.frame(colData(vsd_filtered)[, c("kit_name", "filter_name")]) %>%
...
colnames(mat) <- (vsd_filtered)$sample_short
rownames(anno) <- (vsd_filtered)$sample_short
# Adapted from: https://slowkow.com/notes/pheatmap-tutorial/
mat_colors <- list(
  `Kit Name` = palette.colors(length(unique(anno$`Kit Name`)), palette = "Set 2", alpha = 0.9),
...
This plot cluster the top n=20 most variable genes centred on the gene's average expression level across all samples using the rlog transformed data.

```{r}
# select 20 most variable genes
topVarGenes <- head(order(rowVars(assay(rld_filtered)), decreasing = TRUE), 20)
# select rlog transformed data and center
mat <- assay(rld_filtered)[topVarGenes, ]
mat <- mat - rowMeans(mat)
# add grouping columns
anno <- as.data.frame(colData(rld_filtered)[, c("kit_name", "filter_name")]) %>%
  mutate(kit_name = as.factor(kit_name)) %>%
  mutate(filter_name = as.factor(filter_name)) %>%
  rename_with(~ str_to_title(str_replace(.x, "_", " ")))
# set names of samples
colnames(mat) <- rld_filtered$sample_short
rownames(anno) <- rld_filtered$sample_short
# Adapted from: https://slowkow.com/notes/pheatmap-tutorial/
mat_colors <- list(
  `Kit Name` = palette.colors(length(unique(anno$`Kit Name`)), palette = "Set 2", alpha = 0.9),
  `Filter Name` = palette.colors(length(unique(anno$`Filter Name`)), palette = "Okabe-Ito", alpha = 0.9)
...
```{r}
# select 20 most variable genes Pk protein coding genes
topVarGenes <- head(
  order(
    rowVars(
      assay(vsd_filtered[rowData(vsd_filtered)$species == "pk" & rowData(vsd_filtered)$gene_biotype == "protein_coding"])
    ),
    decreasing = TRUE
  ),
  20
...
# select vst transformed data and center
mat <- assay(vsd_filtered[rowData(vsd_filtered)$species == "pk" & rowData(vsd_filtered)$gene_biotype == "protein_coding"])[topVarGenes, ]
mat <- mat - rowMeans(mat)
# add grouping columns
anno <- as.data.frame(colData(vsd_filtered)[, c("kit_name", "filter_name")]) %>%
  mutate(kit_name = as.factor(kit_name)) %>%
...
topVarGenes <- head(
  order(
    rowVars(
      assay(rld_filtered[rowData(rld_filtered)$species == "pk" & rowData(rld_filtered)$gene_biotype == "protein_coding"])
    ),
    decreasing = TRUE
  ),
  20
...
# select vst transformed data and center
mat <- assay(rld_filtered[rowData(rld_filtered)$species == "pk" & rowData(rld_filtered)$gene_biotype == "protein_coding"])[topVarGenes, ]
mat <- mat - rowMeans(mat)
# add grouping columns
anno <- as.data.frame(colData(rld_filtered)[, c("kit_name", "filter_name")]) %>%
  mutate(kit_name = as.factor(kit_name)) %>%
...
<!-- TODO - compare with transform after subsetting -->

<!-- TODO - compare with plotting means (needs size factors to be estimated) -->

<!-- ```{r} -->
<!-- select <- order(rowMeans(counts(estimateSizeFactors(gse_filtered), normalized = FALSE)), -->
<!--   decreasing = TRUE -->
<!-- )[1:20] -->
<!-- df <- as.data.frame(colData(gse_filtered)[, c("RNA_type", "removal", "WBC_depletion")]) -->
<!-- pheatmap(assay(vsd_filtered)[select, ], -->
<!--   cluster_rows = FALSE, show_rownames = FALSE, -->
<!--   cluster_cols = FALSE, annotation_col = df -->
<!-- ) -->
<!-- ``` -->

### Pk genes - protein coding - vst - top expressed genes

This plot cluster the top n=20 most highly expressed Pk protein coding genes (not centred on the gene's average expression level) across all samples using the vst transformed data.

```{r}
# select 20 highest expressed Pk protein coding genes
select <- order(
  rowMeans(counts(
    gse_filtered[rowData(gse_filtered)$species == "pk" & rowData(gse_filtered)$gene_biotype == "protein_coding"],
    normalized = FALSE
  )),
  decreasing = TRUE
)[1:20]
# select vst transformed data without centering
mat <- assay(vsd_filtered[rowData(vsd_filtered)$species == "pk" & rowData(vsd_filtered)$gene_biotype == "protein_coding"])[select, ]
# mat  <- mat - rowMeans(mat)
# add grouping columns
anno <- as.data.frame(colData(vsd_filtered)[, c("kit_name", "filter_name")]) %>%
  mutate(kit_name = as.factor(kit_name)) %>%
...
### Pk genes - protein coding - rlog - top expressed genes

This plot cluster the top n=20 most highly expressed Pk protein coding genes (not centred on the gene's average expression level) across all samples using the rlog transformed data.
...
# select vst transformed data without centering
mat <- assay(rld_filtered[rowData(rld_filtered)$species == "pk" & rowData(rld_filtered)$gene_biotype == "protein_coding"])[select, ]
# mat  <- mat - rowMeans(mat)
# add grouping columns
anno <- as.data.frame(colData(rld_filtered)[, c("kit_name", "filter_name")]) %>%
  mutate(kit_name = as.factor(kit_name)) %>%
...
This plot cluster the top n=20 most highly expressed Pk genes (not centred on the gene's average expression level) across all samples using the vst transformed data, without clustering.

The data was transformed after subsetting, which is not the recommended approach.
...
This plot cluster the top n=20 most highly expressed Pk genes (not centred on the gene's average expression level) across all samples using the rlog transformed data, without clustering.
...
# select vst transformed data without centering
mat <- assay(rld_pk_filtered)[select, ]
# mat  <- mat - rowMeans(mat)
# add grouping columns
anno <- as.data.frame(colData(rld_pk_filtered)[, c("kit_name", "filter_name")]) %>%
  mutate(kit_name = as.factor(kit_name)) %>%
  mutate(filter_name = as.factor(filter_name)) %>%
  rename_with(~ str_to_title(str_replace(.x, "_", " ")))
# set names of samples
colnames(mat) <- rld_pk_filtered$sample_short
rownames(anno) <- rld_pk_filtered$sample_short
# Adapted from: https://slowkow.com/notes/pheatmap-tutorial/
mat_colors <- list(
  `Kit Name` = palette.colors(length(unique(anno$`Kit Name`)), palette = "Set 2", alpha = 0.9),
  `Filter Name` = palette.colors(length(unique(anno$`Filter Name`)), palette = "Okabe-Ito", alpha = 0.9)
