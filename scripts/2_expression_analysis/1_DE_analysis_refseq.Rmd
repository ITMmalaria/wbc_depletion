...
mat <- round(
          cor(
            gene_tpm_filtered %>%
              ...
              filter(gene_biotype == "protein_coding") %>% 
              select(`104974-001-001_S1_L001`:`104974-001-016_S1_L001`) %>% 
              rename_with(~samples %>% pull(sample_short) %>% as.character(), everything())
            ),
          1)
corr <- mat
...
ggsave(here("results-refseq/figures/correlation_protein_coding_genes.png"), height = 3000, width = 3000, units = "px")
```

<!-- ```{r correlation-plot-tester, message=FALSE, cache=TRUE, fig.height=18, out.width="100%"} -->
<!-- # ggpairs( -->
<!-- #   gene_tpm_filtered %>% -->
<!-- #     filter(species == "pk") %>% -->
<!-- #     filter(gene_biotype == "protein_coding"), -->
<!-- #   columns = 3:5, # `104974-001-001_S1_L001`:`104974-001-002_S1_L001`, -->
<!-- #   lower = list(continuous = "smooth"), -->
<!-- #   upper = list(continuous = wrap("cor", size = 2)), -->
<!-- #   columnLabels = samples$sample_short[1:3] -->
<!-- # ) -->
...
ggsave(here("results-refseq/figures/correlation_protein_coding_genes-logscaled.png"), height = 3000, width = 3000, units = "px")
```
...
### Alternative ordered by library - depletion - filter 

```{r combined-figure-ordered-alternative, message = FALSE}
samples_order <- samples %>% 
  arrange(factor(samples$kit, levels = c("mRNA_illumina", "mRNA_qiagen_FSglobH", "mRNA_qiagen_FSglob", "tRNA_qiagen_FSglobH")),
          factor(samples$WBC_depletion, levels = c("nofilter", "centpip", "plasmo", "pmacs", "cellulose"))) %>% 
  select(sample_name)
...
p2_c <- gene_counts_txi_filtered_long %>%
  ...
  scale_y_discrete(limits = rev) +
  scale_x_continuous(labels = scales::label_number(scale_cut = cut_short_scale())) +
  labs(y = "Sample", x = expression(paste("Sum of ", italic("P. knowlesi"), " protein-coding gene counts")), fill = "RNA type") +
  theme_classic() +
  theme(axis.title.y = element_blank(), axis.text.y = element_blank()) +
  ggtitle("C")
